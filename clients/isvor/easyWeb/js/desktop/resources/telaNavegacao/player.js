                    ...
                    if (endIndex == recurso.getSlideNumber()) {
                        ...
                        */
                        ...
                        if (Player.Scorm.vars["cmi.suspend_data"].t == undefined) {
                            ...
                            $textoDinamico.text("Escolha o próximo módulo que deseja realizar. Lembre-se de realizar todos os módulos obrigatórios para aprimorar seus conhecimentos e poder realizar a atividade avaliativa do curso.");
            

                        }else if(Player.Scorm.vars["cmi.suspend_data"].t.obC.length == 3 && Player.Scorm.vars["cmi.suspend_data"].t.opC.length < 2){
                            $textoDinamico.text("Muito bem! Você já realizou todos os módulos obrigatórios deste curso. Agora você já pode realizar a atividade avaliativa deste curso. Mas lembre-se, esta avaliação irá abordar todos os tópicos deste curso, por isso não deixe de estudar os módulos opcionais caso não esteja seguro em todos os tópicos abordados");
                        }else if(Player.Scorm.vars["cmi.suspend_data"].t.obC.length == 3 && Player.Scorm.vars["cmi.suspend_data"].t.opC.length == 2){
